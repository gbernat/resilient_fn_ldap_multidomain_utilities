...
class FunctionComponent(ResilientComponent):
    """Component that implements Resilient function 'ldap_md_utilities_toggle_access''"""
    ...
    @function("ldap_md_utilities_toggle_access")
    def _ldap_md_utilities_toggle_access_function(self, event, *args, **kwargs):
        ...
        try:
            ...
            yield StatusMessage("Starting 'ldap_md_utilities_toggle_access' running in workflow '{0}'".format(wf_instance_id))

            # Get the function parameters:
            ldap_md_domain_name = kwargs.get("ldap_md_domain_name")  # text
            ldap_md_toggle_access = self.get_select_param(kwargs.get("ldap_md_toggle_access"))  # select, values: "Enable", "Disable"
            ldap_md_dn = kwargs.get("ldap_md_dn")  # text

            log = logging.getLogger(__name__)
            log.info("ldap_md_domain_name: %s", ldap_md_domain_name)
            log.info("ldap_md_toggle_access: %s", ldap_md_toggle_access)
            ...
            if (ldap_md_toggle_access.lower() == 'enable'):
              ldap_user_accout_control_value = 512
              user_status = "Enabled"

            elif (ldap_md_toggle_access.lower() == 'disable'):
            ...
            yield StatusMessage("Finished 'ldap_md_utilities_toggle_access' that was running in workflow '{0}'".format(wf_instance_id))
